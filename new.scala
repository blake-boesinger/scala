...
object Main {
  ...
  def minimum[F[_], A](as: F[A], order: Order[A], fold: Foldable[F]) : Option[A] ={
    fold.reducel[A]( (a:A,b:A) => order.min(b,a)    ,as)
  }
  ...
  def main(args: Array[String]) {
    ...
    val j = minimum(List(1, 8, 2, 9), IntOrder, ListFoldable)
    println(j)    
  }
}